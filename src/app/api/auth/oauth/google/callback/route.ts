...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  await initializeDatabase();
    ...
    ).get("google", profile.providerUserId) as OAuthAccount | undefined;
    ...
    // Check if email already used with a different method
    const existingUser = await db.prepare("SELECT id FROM users WHERE email = ?").get(profile.email);
    if (existingUser) {
      return NextResponse.redirect(`${BASE_URL}/login?error=oauth_email_conflict&provider=google`);
    }

    // New user — store pending data and redirect to role selection
    const pendingJwt = await signPendingJwt({ email: profile.email, name: profile.name, provider: "google", providerUserId: profile.providerUserId, emailVerified: true });

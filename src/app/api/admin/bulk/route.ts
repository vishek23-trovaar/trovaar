...
export async function POST(request: NextRequest) {
  ...
  if (type === "jobs") {
    // Jobs bulk actions
    if (action === "delete") {
      await db.prepare(`DELETE FROM bids WHERE job_id IN (${placeholders})`).run(...ids);
    ...
    if (action === "suspend") {
      await db.prepare(`UPDATE contractor_profiles SET is_suspended = 1 WHERE user_id IN (${placeholders})`).run(...ids);
    } else if (action === "unsuspend") {
      await db.prepare(`UPDATE contractor_profiles SET is_suspended = 0 WHERE user_id IN (${placeholders})`).run(...ids);

...
export async function GET(request: NextRequest) {
  ...
  if (type === "users") {
    rows = await db.prepare(`SELECT u.id, u.name, u.email, u.phone, u.role, u.created_at,
      u.is_suspended, u.account_number,
      COUNT(DISTINCT j.id) as total_jobs
      FROM users u
      LEFT JOIN jobs j ON (j.consumer_id = u.id OR j.contractor_id = u.id)
      GROUP BY u.id ORDER BY u.created_at DESC`).all() as Record<string, unknown>[];
    filename = "users.csv";
  } else if (type === "consumers") {
    rows = await db.prepare(`SELECT u.id, u.name, u.email, u.phone, u.created_at, u.is_suspended,
      u.account_number,
    ...
    rows = await db.prepare(`SELECT u.id, u.name, u.email, u.phone, u.created_at, u.is_suspended,
      u.account_number,
      cp.rating, cp.completed_jobs, cp.id_verified, cp.insurance_verified,
    ...
    rows = await db.prepare(`SELECT j.id, j.title, j.category, j.status, j.urgency, j.location,
      ...
      GROUP BY j.id ORDER BY j.created_at DESC`).all() as Record<string, unknown>[];
    filename = "jobs.csv";
  } else if (type === "revenue") {
    rows = await db.prepare(`SELECT
      ...
      GROUP BY DATE(b.created_at) ORDER BY date DESC`).all() as Record<string, unknown>[];

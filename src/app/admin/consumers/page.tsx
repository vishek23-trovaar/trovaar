  ...
  return (
    <div className="p-8 max-w-7xl">
      ...
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-[#d0d6e0]">Consumers</h1>
          <p className="text-[#8a8f98] text-sm mt-1">{total.toLocaleString()} registered clients</p>
        </div>
        <a
      ...
      <div className="bg-[#0f1011] rounded-2xl border border-[#23252a] shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-16 text-[#8a8f98]">No consumers found.</div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#23252a] bg-[#010102]">
                  ...
                  <th className="text-left px-6 py-3 text-xs font-medium text-[#8a8f98]">Contact</th>
                  <th className="text-left px-6 py-3 text-xs font-medium text-[#8a8f98]">Verified</th>
              ...
              <tbody>
                {users.map((u) => (
                  <tr key={u.id} className="border-b border-[#23252a] hover:bg-[#141516] transition-colors">
                    ...
                    <td className="px-6 py-3">
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className="font-medium text-[#d0d6e0]">{u.name}</span>
                      </div>
                    </td>
                    <td className="px-6 py-3">
                      {u.account_number
                    ...
                    <td className="px-6 py-3 text-xs text-[#8a8f98]">
                      <p>{u.email}</p>
                      {u.phone && <p className="text-[#8a8f98] mt-0.5">{u.phone}</p>}
                    </td>
                    <td className="px-6 py-3">
                      <div className="flex flex-col gap-1">
                        <span className={`text-xs font-medium ${u.email_verified ? "text-[#34d399]" : "text-[#fbbf24]"}`}>
                          {u.email_verified ? "Email verified" : "Email unverified"}
                        </span>
                        <span className={`text-xs font-medium ${u.phone_verified ? "text-[#34d399]" : "text-[#8a8f98]"}`}>
                          {u.phone_verified ? "Phone verified" : "Phone unverified"}
                        </span>
                      </div>
                    </td>
                    <td className="px-5 py-3">
                      <span className="text-[#d0d6e0] font-semibold">{u.total_jobs ?? 0}</span>
                    </td>
                    <td className="px-5 py-3">
                      <span className="text-[#d0d6e0] font-semibold">{fmtMoney(u.total_spent_cents)}</span>
                    </td>
                    <td className="px-6 py-3 text-xs text-[#8a8f98]">{fmtDate(u.created_at)}</td>
                    <td className="px-6 py-3">
                      {u.is_suspended
                        ? <span className="text-xs bg-[#f87171]/10 text-[#f87171] px-2 py-0.5 rounded-full font-medium">Suspended</span>
                        : <span className="text-xs bg-[#27a644]/10 text-[#34d399] px-2 py-0.5 rounded-full font-medium">Active</span>}
                    </td>
        ...
        {pages > 1 && (
          <div className="flex items-center justify-between px-6 py-4 border-t border-[#23252a]">
            <p className="text-xs text-[#8a8f98]">Page {page} of {pages} &middot; {total.toLocaleString()} consumers</p>
